...
async function listCourses(auth) {
  ...
  console.log(result.data.students[0]);
...
//   const classroom = google.classroom({ version: 'v1', auth });
//   const res = await classroom.courses
// }
...
const course = {
  ...
  description: "We'll be learning about",
  ownerId: "me",
};

async function createCourse(auth) {
  const classroom = google.classroom({ version: "v1", auth });
  const res = classroom.courses.create({ requestBody: course });
  console.log(res);
}

// authorize().then(createCourse).catch(console.error);
